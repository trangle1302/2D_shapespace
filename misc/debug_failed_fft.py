...
print(
    f"Found {len(imlist_filtered)/len(imlist)}% ({len(imlist_filtered)}/{len(imlist)}) segmentations with size >0"
)

imlist_coefs = []
with open(
    "/data/2Dshapespace/S-BIAD34/fftcoefs/fft_major_axis_polarized_ud/fftcoefs_128.txt"
) as f:
    for line in f:
