...
def train(train_files, test_files, save_dir, initial_model="nuclei"):
    ...
    print(
        f">>> average precision at iou threshold 0.5 in test set = {ap[:,0].mean():.3f}"
    )
...
if __name__ == "__main__":
    ...
    model_name = "nuclei"  # 'cyto'
    if model_name == "nuclei":
        train_files = glob(f"{base_dir}/train/*_w1.tif")
        train_files = [
            f
            for f in train_files
            if os.path.exists(f.replace("w1.tif", "nucleimask.png"))
        ]
        ...
        train_files = [
            f
            for f in train_files
            if os.path.exists(f.replace("w1.tif", "cellmask.png"))
        ]
        test_files = glob(f"{base_dir}/test/*_w1.tif")

    train(train_files, test_files, save_dir=base_dir, initial_model=model_name)
