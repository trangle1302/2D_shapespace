...
def main():
    ...
    print(
        pts_avg.max(), pts_avg.min(), cell_contour[:, 0].max(), cell_contour[:, 1].max()
    )
    shape_x, shape_y = (
    ...
    mappings = pd.read_csv(
        "/data/kaggle-dataset/publicHPA_umap/results/webapp/sl_pHPA_15_0.05_euclidean_100000_rmoutliers_ilsc_3d_bbox_rm_border.csv"
    )
    ...
    for i, bin_ in enumerate(merged_bins):
        ...
        for img_id in tqdm(ls):
            ...
            pts_ori = image_warp.find_landmarks(
                nu_resized, cell_resized, n_points=32, border_points=False
            )
