        ...
        for ab_id in ab_keep:
            ...
            for img_id in tqdm(ls_, desc=f"{PC}_bin{bin_[0]}_{ab_id}", total=len(ls_)):
                ...
                if np.random.choice([True,False], p=[0.001,0.999]):
                    ...
                    ax[3].scatter(pts_ori[:,1], pts_ori[:,0], c=np.arange(len(pts_ori)),cmap='Reds')
                    ax[3].set_title('resized protein channel')
                    ax[4].imshow(warped1)
                    ax[4].scatter(pts_convex[:,1], pts_convex[:,0], c=np.arange(len(pts_ori)),cmap='Reds')
                    ax[4].set_title('ori_shape to midpoint')
                    ax[5].imshow(warped)
                    ax[5].scatter(pts_avg[:,1], pts_avg[:,0], c=np.arange(len(pts_ori)),cmap='Reds')
