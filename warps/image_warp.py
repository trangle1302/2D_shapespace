...
def warp_image(pts_from, pts_to, img, midpoint=False, plot=True, save_dir=""):
    ...
    if midpoint:
        ...
        transform1 = TPSpline._make_inverse_warp(
            pts_from, midpoint, (0, 0, x_max, y_max), approximate_grid=2
        )
        transform2 = TPSpline._make_inverse_warp(
            midpoint, pts_to, (0, 0, x_max, y_max), approximate_grid=2
        )
        warped1 = cv2.remap(
            img,
            transform1[1].astype("float32"),
            transform1[0].astype("float32"),
            cv2.INTER_CUBIC,
        )
        warped = cv2.remap(
            warped1,
            transform2[1].astype("float32"),
            transform2[0].astype("float32"),
            cv2.INTER_CUBIC,
        )
        if plot:
            ...
            ax[0].imshow(img)
            ax[0].scatter(
                pts_from[:, 1], pts_from[:, 0], c=np.arange(len(pts_from)), cmap="Reds"
            )
            ax[0].set_title("protein channel")
            ax[1].imshow(warped1)
            ax[1].scatter(
                midpoint[:, 1], midpoint[:, 0], c=np.arange(len(pts_from)), cmap="Reds"
            )
            ax[1].set_title("ori_shape to midpoint")
            ax[2].imshow(warped)
            ax[2].scatter(
                pts_to[:, 1], pts_to[:, 0], c=np.arange(len(pts_to)), cmap="Reds"
            )
            ax[2].set_title("midpoint to avg_shape")
    else:
        transform = TPSpline._make_inverse_warp(
            pts_from, pts_to, (0, 0, x_max, y_max), approximate_grid=2
        )
        # print("Max y: ", transform[1].astype('float32').max(), "Max x: ", transform[0].astype('float32').max(), img.max())
        warped = cv2.remap(
            img,
            transform[1].astype("float32"),
            transform[0].astype("float32"),
            cv2.INTER_CUBIC,
        )
    return warped
