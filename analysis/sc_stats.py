...
def get_sc_statistics_fucci(cell_mask, nuclei_mask, mt, gmnn, cdt1, protein, cell_mask_path):    
...
def main():
    ...
    with open(f"{d}/single_cell_statistics.csv","a") as f:
        ...
        for cell_mask_path in cell_masks: 
            ...
            nuclei_mask = imageio.imread(cell_mask_path.replace("cellmask","nucleimask"))
            ab_id = cell_mask_path.split('/')[-2]
            img_id = cell_mask_path.split('/')[-1].replace("_cellmask.png","")
            ...
            lines = get_sc_statistics_fucci(cell_mask, nuclei_mask, mt, gmnn, cdt1, protein, cell_mask_path)
            f.writelines(lines)
