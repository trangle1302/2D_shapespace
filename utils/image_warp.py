...
def find_landmarks(nuclei, cell, n_points=32, border_points = False):
    ...
    cell = np.zeros((cell.shape[0]+2, cell.shape[1]+2))
    cell[1:1+cell.shape[0],1:1+cell.shape[1]] = cell
...
def warp_image(pts_from, pts_to, img, plot=True, save_dir=""):
    midpoint = (pts_from + pts_to) /2
    transform1 = TPSpline._make_inverse_warp(pts_from, midpoint, (0, 0, 600,400), approximate_grid=4)
    transform2 = TPSpline._make_inverse_warp(midpoint, pts_to, (0, 0, 600,400), approximate_grid=4)
